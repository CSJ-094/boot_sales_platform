...
프로젝트 제작 당시 마주했던 기술적 문제들과 이를 해결하기 위해 고민했던 포인트들입니다.

### 1. 동적 검색 조건에 따른 쿼리 복잡도 해소
* **Problem**: 검색 조건(상품명, 상태, 등록자 등)이 늘어남에 따라 JPQL이나 MyBatis 방식으로는 쿼리 생성이 매우 복잡해지고 오타로 인한 런타임 에러가 빈번했습니다.
* **Solution**: **QueryDSL**을 도입하여 자바 코드로 쿼리를 작성했습니다. 이를 통해 **컴파일 시점의 타입 체크**가 가능해졌고, 코드의 재사용성과 가독성을 획기적으로 높였습니다.

### 2. 대량 데이터 조회 시 페이지 로딩 성능 최적화
* **Problem**: 메인 페이지에서 모든 상품의 데이터를 한꺼번에 불러올 경우, 상품 수에 비례하여 메모리 부하와 네트워크 전송 지연이 발생했습니다.
* **Solution**: Spring Data의 **Pageable** 인터페이스를 활용해 서버 사이드 페이징을 구현했습니다. 또한, **대표 이미지 플래그(`repImgYn`)**를 설계하여 메인 리스트 조회 시 필요한 최소한의 데이터만 Fetch 하도록 최적화했습니다.

### 3. 실시간 양방향 통신을 통한 사용자 경험(UX) 개선
* **Problem**: 일반적인 HTTP 요청 방식은 새로고침 없이 관리자와 유저가 대화를 주고받는 실시간성을 보장하기 어려웠습니다.
* **Solution**: **WebSocket 및 STOMP** 프로토콜을 활용하여 실시간 상담 기능을 구축했습니다. 메시지 브로커를 통해 메시지를 즉각적으로 전달함으로써, 이커머스 서비스에 필수적인 실시간 고객 대응 환경을 구현했습니다.
